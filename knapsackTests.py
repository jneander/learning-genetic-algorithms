...
class Fitness:
    ...
    def __str__(self):
        return "wt: {:0.2f} vol: {:0.2f} value: {}".format(self.TotalWeight, self.TotalVolume, self.TotalValue)
...
def mutate(genes, items, maxWeight, maxVolume, window):
    ...
    if maxQuantity > 0:
        quantity = maxQuantity if window.Size > 1 else random.randint(1, maxQuantity)
...
def display(candidate, startTime):
    ...
    genes.sort(key=lambda iq: iq.Quantity, reverse=True) # sort ascending by quantity
